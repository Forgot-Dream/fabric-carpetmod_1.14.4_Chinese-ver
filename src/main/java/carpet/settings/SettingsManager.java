...
public class SettingsManager
{
    ...
    private ParsedRule<?> contextRule(CommandContext<ServerCommandSource> ctx) throws CommandSyntaxException
    {
        ...
        if (rule == null)
            throw new SimpleCommandExceptionType(Messenger.c("rb 未知功能: "+ruleName)).create();
        return rule;
    }
    ...
    private int displayRuleMenu(ServerCommandSource source, ParsedRule<?> rule)
    {
        ...
        {
            tags.add(Messenger.c("c ["+t+"]", "^g 列出所有 "+t+" 设置","!/"+identifier+" list "+t));
    ...
    private int setRule(ServerCommandSource source, ParsedRule<?> rule, String newValue)
    {
        if (rule.set(source, newValue) != null)
            Messenger.m(source, "w "+rule.toString()+", ", "c [设为默认?]",
                    "^w 单击以将设置保留在carpet.conf中，以在重新启动后重载",
    ...
    private int setDefault(ServerCommandSource source, ParsedRule<?> rule, String stringValue)
    {
        ...
        Messenger.m(source ,"gi 功能 "+ rule.name+" 现在默认设置为 "+ stringValue);
        return 1;
    }
    ...
    {
        listSettings(source, "所有 "+fancyName+" 设置", getNonDefault());

        if (version != null)
            Messenger.m(source, "g "+fancyName+" 版本: "+ version);
        try
        {
            PlayerEntity player = source.getPlayer();
