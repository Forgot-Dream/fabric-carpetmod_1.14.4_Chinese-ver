...
public class TickSpeed
{
    ...
    public static BaseText tickrate_advance(PlayerEntity player, int advance, String callback, ServerCommandSource source)
    {
        if (0 == advance)
        {
            ...
            finish_time_warp();
            return Messenger.c("gi 时间修改终止");
        }
        if (time_bias > 0)
        {
            String who = "另一个命令";
            if (time_advancerer != null) who = time_advancerer.getEntityName();
            return Messenger.c("l "+who+" 正在运行一条命令，请稍后执行本命令");
        }
    ...
    public static void finish_time_warp()
    {
        ...
        if (tick_warp_callback != null)
        {
            ...
            catch (Throwable var23)
            {
                ...
                {
                    Messenger.m(time_advancerer, "r 命令返回失败-未知错误： ", "rb /"+tick_warp_callback,"/"+tick_warp_callback);
                }
            }
            tick_warp_callback = null;
            tick_warp_sender = null;
        ...
        if (time_advancerer != null)
        {
            Messenger.m(time_advancerer, String.format("gi ... 时间加速已经完成 平均TPS： %d 平均MSPT: %.2f ",tps, mspt ));
            time_advancerer = null;
        }
        else
        {
            Messenger.print_server_message(CarpetServer.minecraft_server, String.format("... 时间加速已经完成 平均TPS： %d 平均MSPT %.2f ",tps, mspt ));
        }
        time_bias = 0;
