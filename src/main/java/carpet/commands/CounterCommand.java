...
public class CounterCommand
{
    ...
    private static int displayCounter(ServerCommandSource source, String color, boolean realtime)
    {
        ...
        if (counter == null) throw new CommandException(Messenger.s("未知的羊毛颜色: "+color));
    ...
    private static int resetCounter(ServerCommandSource source, String color)
    {
        if (color == null)
        {
            ...
            Messenger.m(source, "w 已重置所有漏斗计数");
        }
        else
        {
            HopperCounter counter = HopperCounter.getCounter(color);
            if (counter == null) throw new CommandException(Messenger.s("未知羊毛颜色"));
            counter.reset(source.getMinecraftServer());
            Messenger.m(source, "w 重置 "+color+" 漏斗计数器");
        }
        return 1;
    }
