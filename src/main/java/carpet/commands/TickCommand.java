...
public class TickCommand
{
    ...
    {
        Messenger.m(source, "w 当前TPS: ",String.format("wb %.1f", TickSpeed.tickrate));
    ...
    private static int toggleFreeze(ServerCommandSource source)
    {
        ...
        {
            Messenger.m(source, "gi 游戏已冻结");
        }
        else
        {
            Messenger.m(source, "gi 游戏恢复运行");
        }
        return 1;
    }
    ...
    private static int toggleSuperHot(ServerCommandSource source)
    {
        ...
        {
            Messenger.m(source,"gi Superhot 模式已经开启");
        }
        else
        {
            Messenger.m(source, "gi Superhot 模式已经关闭");
        }
        return 1;
    }
